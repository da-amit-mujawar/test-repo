...
dag = DAG('builds-b2c-link',
          default_args=default_args,
          description='builds b2c-link dd',
...
# get build  UNCOMMENT IN PRODUCTION
...
data_load = RedshiftOperator(
    task_id='updating-tblmain-tblchild',
    dag=dag,
...
get_count = PythonOperator(
    ...
    op_kwargs={'tablename': Variable.get('var-db-1150', deserialize_json=True)['maintable_name']},
    provide_context=True,
    dag=dag)
...
sql_command = """ EXEC Common.dbo.usp_QCBuildReport @DatabaseID = 1150, @ChangePercentage  = 5 """
...
# run rpt scripts
run_report = RedshiftOperator(
    task_id='running-cap-one-report',
    dag=dag,
...
#start_operator >> fetch_buildinfo >> run_report >> send_notification >> end_operator
start_operator >> fetch_buildinfo >> data_load >> get_count >> build_activate >> Fill_rate_QC_report >> run_report >> send_notification >> end_operator
