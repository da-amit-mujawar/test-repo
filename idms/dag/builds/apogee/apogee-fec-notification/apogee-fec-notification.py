...
def rename_copy_file(**kwargs):
    ...
    ls_report_file_fmt = "idms/{yyyy}-{mm}-{dd}_summary.txt"
    ...
    copy_file(ls_bucket_name, f"""{ls_bucket_name}/{ls_file_name}""", f"""idms/{rename_ls_file_name}""")
    delete_file(ls_bucket_name, f"""{ls_file_name}""")

    copy_source = {
        'Bucket': ls_bucket_name,
        'Key': f'idms/{rename_ls_file_name}'
    }
