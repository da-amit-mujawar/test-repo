...
def get_s3_hook():
    ...
    print('after hook')
    s3bucket = s3_hook.get_bucket('idms-2722-internalfiles')
    for obj in s3bucket.objects.filter(Prefix='Reports/sf_test/'):
        print('in loop')
        print(obj.key)
        if obj.key[-1:] != '/':
            key = obj.key.split('/')[-1]
            source_file = 's3://idms-2722-internalfiles/Reports/sf_test/' + key
            dest_file = 's3://idms-2722-internalfiles/Reports/sf_test_copy/' + key
            print(source_file)
            print(dest_file)
...
s3_copy = S3CopyObjectOperator(
    ...
    source_bucket_name='idms-2722-internalfiles',
    dest_bucket_name='idms-2722-internalfiles',
    #dest_file_extension='csv',
    bulk_copy = True
)

#idms-2722-internalfiles Reports/sf_test/
