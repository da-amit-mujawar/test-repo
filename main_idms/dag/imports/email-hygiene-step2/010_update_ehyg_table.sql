-- Clone Step 1 ehyg_{dbid} table into ehyg_{dbid}_step2 to preserve ehyg_{dbid}
DROP TABLE IF EXISTS dbo.ehyg_{dbid}_step2;
CREATE TABLE dbo.ehyg_{dbid}_step2 as (SELECT * FROM dbo.ehyg_{dbid});
-- Add columns from Engaged table
ALTER TABLE dbo.ehyg_{dbid}_step2 ADD Engaged_Flag char(1);
ALTER TABLE dbo.ehyg_{dbid}_step2 ADD Engaged_Open_Date varchar(8);
ALTER TABLE dbo.ehyg_{dbid}_step2 ADD Engaged_Click_Date varchar(8);
--------------------
UPDATE
    ehyg_{dbid}_step2
SET
...
WHERE
    ehyg_{dbid}_step2.email = exclude_engaged_12_mo.emailaddress;
--======================================================================================
...
ALTER TABLE ehyg_{dbid}_step2 ADD EO_Validation_StatusID varchar(10);
ALTER TABLE ehyg_{dbid}_step2 ADD EO_Validation_Status varchar(50);
ALTER TABLE ehyg_{dbid}_step2 ADD EO_EmailDomainGroupID varchar(10);
ALTER TABLE ehyg_{dbid}_step2 ADD EO_EmailDomainGroup varchar(50);

UPDATE ehyg_{dbid}_step2 
...
UPDATE ehyg_{dbid}_step2 
	...
	from ehyg_{dbid}_EO_Returns where ehyg_{dbid}_EO_Returns.email = ehyg_{dbid}_step2 .email;

--======================================================================================
-- Add ehyg code to table using Case When
ALTER TABLE ehyg_{dbid}_step2  ADD Ehygiene_Code char(1);

UPDATE ehyg_{dbid}_step2  SET Ehygiene_Code = ' ';
UPDATE ehyg_{dbid}_step2 
SET
    Ehygiene_Code = CASE
