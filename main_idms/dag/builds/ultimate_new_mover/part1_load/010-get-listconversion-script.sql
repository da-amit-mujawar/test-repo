...
NUA RedShift ETL | VC1 LIST_UPDATES -- INFOGROUP ULTIMATE NEW MOVER - Weekly (1333) -PART 1 - LOAD - WEEKLY (WEDS)


*/
/*
if Odd Week number,  use Build 21095
if Even week number, use Build 21080
...
--20211217 CB: fix logic to select correct buildid
...
UPDATE tblListLoadStatus
   SET iIsCurrent = 0, dModifiedDate = GETDATE(), cModifiedBy   = '{updateuserid}'
...
UPDATE TOP (1) tblListLoadStatus
   SET iIsCurrent = 1, dModifiedDate = GETDATE(), cModifiedBy   = '{updateuserid}'
  FROM dw_admin.dbo.tblListLoadStatus tblListLoadStatus
 WHERE BuildLoLID IN (SELECt TOP 1 ID FROM dw_admin.dbo.tblBuildLoL WHERE MasterLoLID = @Listid AND BuildID = @Buildid)
   AND iIsCurrent = 0
