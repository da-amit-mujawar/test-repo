...
CASE WHEN C.MAILCONFIDENCE IS NULL THEN LEFT(B.Primary_Mail_Confidence,1) ELSE C.MAILCONFIDENCE   END,
LEFT(B.Ad_Size_LRFS,1),
LEFT(B.BADEMAILS,1),
LEFT(B.BUSINESS_STATUS_CODE_LRFS,1),
LEFT(B.CALL_STATUS_LRFS,1),
LEFT(B.CBSA_LRFS,5),
LEFT(B.DOMAIN_NAME_LRFS,80),
LEFT(B.CDOMAIN,1),
LEFT(B.CENSUS_LRFS,6),
LEFT(B.CINCLUDE,1),
LEFT(B.CITY_AVAILABLE,1),
LEFT(B.COMPANY_NAME_LRFS,30),
LEFT(B.COMPANY_AVAILABLE_LRFS,1),
...
LEFT(B.SALES_VOLUME_CODE_LRFS,1),
LEFT(B.OWN_LEASE_LRFS,1),
LEFT(B.PERMISSIONTYPE,1),
LEFT(B.A10_DIGIT_PHONE_LRFS,10),
LEFT(B.PHONE_NUMBER_AVAILABLE_LRFS,1),
...
LEFT(B.PUBLIC_PRIVATE_CODE_LRFS,1),
...
B.Inferred_Corporate_Sales_Volume_Source_Indicator,B.NAICSCODE,
...
LEFT(B.Primary_Mail_Confidence,1)

FROM business_email_table B
LEFT JOIN 
(
  SELECT ADDRESS,CITY,STATE,ZIP,ZIPFOUR,ZIP9,CARRIERROUTECODE,ABINUMBER,MAILCONFIDENCE 
  FROM tblBusinessCompany
) C
ON B.ABI_Number_LRFS = C.ABINUMBER 
WHERE (B.ContactID IS NULL OR B.ContactID = '')
;


update tblBusinessIndividual 
set PRIMARYSIC2 = B.SIC2,
PRIMARYSIC4 = B.SIC4,
PRIMARYSIC6 = B.SIC6
from tblBusinessIndividual A
INNER JOIN (SELECT ABINUMBER, MIN(PRIMARYSIC2) as SIC2,MIN(PRIMARYSIC4) as SIC4,MIN(PRIMARYSIC6) as SIC6  
   FROM tblBusinessIndividual Where PRIMARYSIC2  is NOT null  Group by ABINUMBER )as B
On A.ABINUMBER = B.ABINUMBER
where A.PRIMARYSIC2 is null or a.PRIMARYSIC2 ='' ;