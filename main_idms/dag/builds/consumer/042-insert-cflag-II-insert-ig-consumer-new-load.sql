...
INNER JOIN {new-load-table} B 
ON A.Individual_ID = B.Individual_ID 
WHERE  A.Individual_ID<>'000000000000' AND 
A.Individual_ID<>'' AND 
A.Individual_ID IS NOT NULL AND 
a.cFlag<>'IU' AND 
A.opt_out_flag='0' AND 
A.EmailAddress NOT IN 
    (
        SELECT EmailAddress FROM {new-load-table} 
        WHERE EmailAddress<>'' AND EmailAddress IS NOT NULL
    ); 


--Insert records for II flag
...
SELECT
    ...
    A.credit_card_code, 
    A.number_trade_lines, 
    A.marital_status, 
    A.spousal_indicator_raw, 
    A.cat_owner, 
    A.dog_owner, 
    ...
    A.last_party_contributed_to, 
    A.aircraft_type_code, 
    A.boat_owner_indicator, 
    A.registered_voter, 
    A.political_party_affiliation, 
    ...
    A.vendorethnicity_ext, 
    A.vendorreligion_ext, 
    A.vendorlanguage_ext, 
    A.vendorethnicgroup_ext, 
    A.countryoforign_ext, 
    ...
    A.gui_probable, 
    A.gui_probable_mightbe, 
    A.gui_accurate, 
    A.telemarketingflag, 
    A.phoneincludeallflag, 
    A.segmentcode_a, 
    A.segmentcode_e, 
    A.segmentcode_f, 
    A.segmentcode_g, 
