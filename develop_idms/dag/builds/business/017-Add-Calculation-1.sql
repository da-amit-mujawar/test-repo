DROP TABLE IF EXISTS tblBusinessIndividual_ctas1 CASCADE;

CREATE TABLE tblBusinessIndividual_ctas1
AS
SELECT
a.ID AS ID,
...
-- This logic is only for CANADA
-- case when a.CBSACODE IS NOT NULL AND a.CBSACODE<>'' then RIGHT(a.CBSACODE, 3) else a.CBSACODE end as CBSACODE,
a.CBSACODE AS CBSACODE,
CASE WHEN a.MAILCONFIDENCE is null OR a.MAILCONFIDENCE = '' then LEFT(a.MAIL_SCORE_Email,1) else a.MAILCONFIDENCE end as MAILCONFIDENCE,
...
CASE WHEN (CASE WHEN a.STATE = 'PR' THEN 'N' ELSE a.haspostal END) ='N' and (a.FULFILLMENTFLAG is Null OR a.FULFILLMENTFLAG ='' )  THEN 'U' ELSE a.FULFILLMENTFLAG END AS FULFILLMENTFLAG,
...
CASE WHEN a.TITLECODE = '!'  THEN 'Z1'
   WHEN a.TITLECODE = '#'  THEN 'Z2'
   WHEN a.TITLECODE = '$'  THEN 'Z3'
   WHEN a.TITLECODE = '%'  THEN 'Z4'
   WHEN a.TITLECODE = '&'  THEN 'Z5'
   WHEN a.TITLECODE = '('  THEN 'Z6'
   WHEN a.TITLECODE = ')'  THEN 'Z7'
   WHEN a.TITLECODE = '+'  THEN 'Z8'
   WHEN a.TITLECODE = '-'  THEN 'Z9'
   WHEN a.TITLECODE = '.'  THEN 'Z10'
   WHEN a.TITLECODE = '/'  THEN 'Z11'
   WHEN a.TITLECODE = ':'  THEN 'Z12'
   WHEN a.TITLECODE = '='  THEN 'Z13'
   WHEN a.TITLECODE = '>'  THEN 'Z14'
   WHEN a.TITLECODE = '?'  THEN 'Z15'
   WHEN a.TITLECODE = '@'  THEN 'Z16'
   WHEN a.TITLECODE = '['  THEN 'Z17'
   WHEN a.TITLECODE = '\\'  THEN 'Z18'
   WHEN a.TITLECODE = ']'  THEN 'Z19'
   WHEN a.TITLECODE = '^'  THEN 'Z20'
   WHEN a.TITLECODE = '_'  THEN 'Z21'
   WHEN a.TITLECODE = '{'  THEN 'Z22'
   ELSE NVL(a.TITLECODE,'') END as TITLECODE,
...
a.HIGHINCOMEEXECFLAG AS HIGHINCOMEEXECFLAG,
...
-- CANADA Logic
-- CASE WHEN a.STATE is not null and a.DISTRICTCODE is not null then NVL(a.STATE,'')+NVL(a.DISTRICTCODE,'') else NVL(a.DISTRICTCODE,'') end as DISTRICTCODE,
...
CASE WHEN a.STATE = 'PR' THEN 'N' ELSE a.haspostal END AS Haspostal,
11318 as Listid,
'R' as PermissionType,
...
CASE WHEN a.EmailAddress is not null and a.EmailAddress <> '' and UPPER(a.EmailAddress) LIKE '%.CA' 
      THEN '' else NVL(a.EmailAddress,'') end as EmailAddress,
NVL(a.cDomain,'') AS cDomain,
CASE WHEN a.cDomainType = 'YA' THEN 'Y'  
   WHEN a.cDomainType = 'HO' THEN 'H' 
   WHEN a.cDomainType = 'GM' THEN  'G'
   WHEN a.cDomainType = 'AO' THEN  'A'
   WHEN a.cDomainType = 'OT' THEN  'O'
     ELSE NVL(a.cDomainType,'') end as cDomainType,
CASE WHEN a.EmailAddress is not null and a.EmailAddress <> '' and UPPER(a.EmailAddress) LIKE '%.CA' 
         THEN 'Y' 
     else NVL(a.BadEmails,'') end as BadEmails,
NVL(a.Royalty_Emails,'') AS Royalty_Emails,
NVL(a.Infogroup_Emails,'') AS Infogroup_Emails,
CASE WHEN a.EmailAddress is not null and a.EmailAddress <> '' and UPPER(a.EmailAddress) LIKE '%.CA' THEN 'C'
   WHEN a.STATE = 'PR' THEN 'N' 
   else NVL(a.cInclude,'') end as cInclude,
CASE WHEN a.EmailAddress is not null and 
      a.EmailAddress <> '' and 
      UPPER(a.EmailAddress) LIKE '%.CA' 
   THEN '' else NVL(a.MD5Upper,'') end as MD5Upper,
CASE WHEN a.EmailAddress is not null and 
      a.EmailAddress <> '' and 
      UPPER(a.EmailAddress) LIKE '%.CA' 
   THEN '' else NVL(a.MD5Lower,'') end as MD5Lower,
CASE WHEN a.EmailAddress is not null and 
      a.EmailAddress <> '' and 
      UPPER(a.EmailAddress) LIKE '%.CA' 
   THEN '' else NVL(a.MD5SaltedHash,'') end as MD5SaltedHash,
...
CASE WHEN a.CONTACTID is NULL then ''
     WHEN NVL(a.LiteralTitle_ID,'') = i.cCode then NVL(i.cDescription,'') 
...
LEFT(a.ZIP_Back,3) as SCF,
CASE WHEN LEN(a.ZIP_Back) =5 THEN RIGHT(a.ZIP_Back, 1) ELSE '' END as ZIP_END,
...
NVL(LTRIM(RTRIM(a.ZIP_Back)),'') + NVL(LTRIM(RTRIM(a.CARRIERROUTECODE)),'') AS ZipCarrierRoute,
...
REPLACE(NVL(a.ADDRESS,'') + NVL(a.CITY,'') + NVL(a.STATE,'') + NVL(a.ZIP_Back,''), ' ', '') AS HardKey,
...
NVL(a.STATE,'') + NVL(LTRIM(RTRIM(UPPER(a.CITY))),'') + NVL(a.ZIP_Back,'') AS StateCityZip,
NVL(a.STATE,'') + NVL(LTRIM(RTRIM(UPPER(a.CITY))),'') + NVL(LEFT(a.ZIP_Back,3),'') AS StateCitySCF,
...
--NVL(a.AMI_HospitalName,'') AS AMI_HospitalName,
...
ltrim(rtrim(nvl(ISNULL(h.City, a.City),'')))+ ' '+ ltrim(rtrim(NVL(a.STATE,''))) as PreferredCity,
--replace(h.DISTRICT_DESCRIPTION, h.STATE_DESCRIPTION, '' ) as DISTRICT_DESCRIPTION,
--h.CREDIT_RATING_DESCRIPTION as CREDIT_RATING_DESCRIPTION,
--h.TITLE_DESCRIPTION as TITLE_DESCRIPTION,
--h.GENDER_DESCRIPTION as GENDER_DESCRIPTION,
--h.CMA_DESCRIPTION as CMA_DESCRIPTION,
a.RegionCode_Multi AS RegionCode_Multi,
-- case when a.ZIP LIKE 'B3%'
--         OR LEFT(a.ZIP,3) BETWEEN 'B2V' AND 'B2Z'  
--         OR LEFT(a.ZIP,3) BETWEEN 'B4A' AND 'B4G' then 'RA' else '' end as RegionCode_01,
-- case when a.ZIP LIKE 'G0A%'  
--     OR a.ZIP LIKE 'G2%'  
--     OR LEFT(a.ZIP,3) BETWEEN 'G1A' AND 'G1Z'  
--     OR LEFT(a.ZIP,3) BETWEEN 'G3A' AND 'G3K' 
--     OR LEFT(a.ZIP,3) BETWEEN 'G6V' AND 'G6X' then 'RB' else '' end as RegionCode_02,
-- case when LEFT(a.ZIP,3) = 'J8X'  
--         OR a.ZIP LIKE 'K0A%'  
--         OR a.ZIP LIKE 'K1%' 
--         OR a.ZIP LIKE 'K2%' 
--         OR a.ZIP LIKE 'K4A%' then 'RC' else '' end as RegionCode_03,
-- case when a.ZIP LIKE 'R3%'  
--             OR a.ZIP LIKE 'R4A%' 
--             OR LEFT(a.ZIP,3) BETWEEN 'R2C' AND 'R2Z' then 'RD' else '' end as RegionCode_04,
-- case when LEFT(a.ZIP,3) BETWEEN 'T1X' AND 'T3Z'  then 'RE' else '' end as RegionCode_05,
-- case when LEFT(a.ZIP,3) BETWEEN 'T5A' AND 'T6X' then 'RF' else '' end as RegionCode_06,
...
-- case when a.ZIP LIKE 'M%'  
--           OR LEFT(a.ZIP,3) BETWEEN 'L1S' AND 'L1Z'  
--           OR LEFT(a.ZIP,3) BETWEEN 'L3P' AND 'L3T' 
--           OR LEFT(a.ZIP,3) BETWEEN 'L4B' AND 'L4L' 
--           OR LEFT(a.ZIP,3) BETWEEN 'L4S' AND 'L5W'
--           OR LEFT(a.ZIP,3) BETWEEN 'L6B' AND 'L7A' then 'R3' else '' end as RegionCode_09,
-- case when a.ZIP LIKE 'L0B%'
--         OR a.ZIP LIKE 'L0C%'
--         OR a.ZIP LIKE 'L0G%'
--         OR a.ZIP LIKE 'L0H%'
--         OR a.ZIP LIKE 'L0J%'
...
--         OR a.ZIP LIKE 'L5%'
--         OR a.ZIP LIKE 'L6%'
--         OR a.ZIP LIKE 'M%' then 'R4' else '' end as RegionCode_10,
-- case when a.ZIP LIKE 'M%'
--             OR LEFT(a.ZIP,3) BETWEEN 'L1G' AND 'L1Z'
--             OR LEFT(a.ZIP,3) BETWEEN 'L2A' AND 'L3M'
--             OR LEFT(a.ZIP,3) BETWEEN 'L3P' AND 'L3T'
--             OR LEFT(a.ZIP,3) BETWEEN 'L4B' AND 'L4L'
--             OR LEFT(a.ZIP,3) BETWEEN 'L4S' AND 'L5W'
--             OR LEFT(a.ZIP,3) BETWEEN 'L6B' AND 'L7A'
--             OR LEFT(a.ZIP,3) BETWEEN 'L7L' AND 'L9K' then 'R5' else '' end as RegionCode_11,
-- case when a.ZIP LIKE 'L%'  OR a.ZIP LIKE 'M%' then 'R6' else '' end as RegionCode_12,
-- case when a.ZIP LIKE 'H%'
--             OR a.ZIP LIKE 'J0L%'
--             OR a.ZIP LIKE 'J0N%'
--             OR a.ZIP LIKE 'J0P%'
--             OR a.ZIP LIKE 'J0R%'
...
--             OR a.ZIP LIKE 'J5%'
--             OR a.ZIP LIKE 'J6%'
--             OR a.ZIP LIKE 'J7%' then 'R7' else '' end as RegionCode_13,
-- case when LEFT(a.ZIP,3) BETWEEN 'V8N' AND 'V9E' then 'R8' else '' end as RegionCode_14,
-- case when a.ZIP LIKE 'V3%'
--         OR a.ZIP LIKE 'V4%'
--         OR a.ZIP LIKE 'V5%'
--         OR a.ZIP LIKE 'V6%'
--         OR a.ZIP LIKE 'V7%'
--         OR LEFT(a.ZIP,3) BETWEEN 'V2S' AND 'V2Z'
--         OR LEFT(a.ZIP,3) BETWEEN 'V1M' AND 'V1M' then 'R9' else '' end as RegionCode_15,
...
LEFT JOIN 
(
   select contactid,cvalue from ApplicationCodes_992_E group by contactid,cvalue
)C 
ON a.ContactID = C.ContactID AND C.cValue = 'H'
LEFT JOIN 
(
   select contactid,cvalue from SegmentCodes_992_E where cValue ='3'
)D 
ON a.ContactID = D.ContactID 
LEFT JOIN 
(
   select contactid,cvalue from SegmentCodes_992_E where cValue ='7'
)e
