UPDATE tblBusinessIndividual --28min 297055422
   SET fLATITUDE  = cast(LEFT(RIGHT('000' + NVL(CAST(cast(fLATITUDE_old as int)  as varchar),''),9),3) + '.' + RIGHT(RIGHT('000' + NVL(CAST(cast(fLATITUDE_old  as int) as varchar),''),9),6) as float),
		...
		One_Per_Company_Name_Title = LEFT(LTRIM(RTRIM(NVL(Company,''))),30)+ LTRIM(RTRIM(NVL(TitleCode_back,''))),
...
set CBSACODE = case when CBSACODE IS NOT NULL AND CBSACODE<>'' then RIGHT(CBSACODE, 3) else CBSACODE end,
TitleCode = CASE WHEN TITLECODE = '!'  THEN 'Z1'
...
DISTRICTCODE = trim(NVL(STATE,''))+trim(NVL(DISTRICTCODE,''));