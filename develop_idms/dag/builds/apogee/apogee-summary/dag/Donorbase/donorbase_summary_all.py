...
def set_emr_process_date(**kwargs):
    ...
    build_info_dic = {}
    ls_process_dt = kwargs["dag_run"].conf.get("process_date") or date.today().strftime(
        "%Y%m%d"
    )
...
dag = DAG(
    "builds-donorbase-rollup-all",
    default_args=default_args,
    description="Calculate Summary Information for DonorBase Data - EMR and DBProcessing",
...
set_emr_prcsdate = PythonOperator(
    task_id="set-process-date", python_callable=set_emr_process_date, dag=dag
)

csv_to_parq = EMROperator(
    task_id="Convert_CSV_2_Parquet",
    base_config_path=ls_current_path + "/emrconfig.json",
    custom_config_path=ls_current_path + "/donorbase-step-0.json",
    spark_config=ls_current_path + "/config.json",
    process_info=Variable.get(ls_summ_calc_var, deserialize_json=True),
    dag=dag,
)

prepare_data = EMROperator(
    task_id="Prepare_Input_Data",
    base_config_path=ls_current_path + "/emrconfig.json",
    custom_config_path=ls_current_path + "/donorbase-step-1.json",
    spark_config=ls_current_path + "/config.json",
    process_info=Variable.get(ls_summ_calc_var, deserialize_json=True),
    dag=dag,
)

list_process = EMROperator(
    task_id="Process_Calc",
    base_config_path=ls_current_path + "/emrconfig.json",
    custom_config_path=ls_current_path + "/donorbase-step-2.json",
    spark_config=ls_current_path + "/config.json",
    process_info=Variable.get(ls_summ_calc_var, deserialize_json=True),
    dag=dag,
)
...
#                              base_config_path=ls_current_path + '/emrconfig.json',
#                              custom_config_path=ls_current_path + '/donorbase-step-3.json',
#                              spark_config=ls_current_path + '/config.json',
#                              process_info=Variable.get(ls_summ_calc_var, deserialize_json=True),
#                              dag=dag)
...
# begin_prcs >> set_emr_prcsdate >> fetch_build >> csv_to_parq >> prepare_data >> list_process >> custom_process >> dwnld_sql >> data_load >> addl_load >> end_prcs
