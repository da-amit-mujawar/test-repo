...
dag = DAG(
    "imports-sic-franchise-tables",
    default_args=default_args,
    description="imports sic-franchise-tables",
...
data_load = RedshiftOperator(
    task_id="loading-imports-sic-franchise-tables",
    dag=dag,
...
data_check = DataCheckOperator(
    ...
    config_file_path=default_args["current_path"] + "/config.json",
    min_expected_count=[
        ("sql_tblsicfranchisecode_new", 4000),
        ("sql_tblindustrycode_new", 700),
        ("tblindustrycode_new", 700)
    ],
    redshift_conn_id=Variable.get("var-redshift-postgres-conn"),
)
