    ...
    sql = f"""
            SELECT MLoL.ID listid, MLoL.cListName listname, LK_Action action, iDropDuplicates dropunique, 
             ...
             INNER JOIN DW_Admin.dbo.tblListLoadStatus s (nolock) 
                ON BLoL.ID = s.BuildLoLID AND s.iIsCurrent = 1     
...
def apogee_input_count(buildid, **kwargs):
    sqlhook = get_sqlserver_hook()
    ...
    with closing(sql_conn.cursor()) as sql_cursor:
        sql = """
            drop table if exists ##Process_Apogee_Step02;
            CREATE TABLE ##Process_Apogee_Step02 ( 
            BuildID int NOT NULL,
            DatabaseID int NULL,          
            OwnerID int NULL,
            ListID int NULL,
            TransListID int NULL,
            Code char(4) NULL,
            ListAction char(1) NULL,
            TransListAction char(1) NULL)
        """
        sql_cursor.execute(sql)

        # CB: Add to join: dw_admin.dbo.tblListLoadStatus LLS on tblBuildLoL.id = LLS.BuildLoLID
        # CB: Add to where: LLS.iIsCurrent = 1 AND LLS.LK_LoadStatus = 120 AND tblMasterLoL.iIsActive=1
        sql = f"""
            SELECT BLoL.buildid, MLoL.DatabaseID, MLoL.OwnerId, MLoL.ID ListID, MLoL.cCode Code, BLoL.LK_Action
             FROM DW_Admin.dbo.tblBuildLoL BLoL (nolock) INNER JOIN                                    
                  DW_Admin.dbo.tblMasterLoL MLoL (nolock) on BLoL.MasterLoLID=MLoL.ID  INNER JOIN
				  DW_Admin.dbo.tblListLoadStatus LLS (nolock) on BLoL.id = LLS.BuildLoLID   
             WHERE BLoL.buildid={buildid} AND MLoL.id not in (16976)--(11126, 6829, 6690, 16975, 16976)
			   AND MLoL.iIsActive=1
               AND BLoL.LK_Action IN ('R','N','A')
               AND MLoL.cCode NOT LIKE '%-S'
			   AND LLS.iIsCurrent = 1 
			   AND LLS.LK_LoadStatus = 120
            ORDER BY MLoL.cCode
        """

        # Fix column order in row
        # Flatten from 1 row per listid, to 1 row per ownerid - Action defaults to DonorList (-C)
        sql_cursor.execute(sql)
        row = sql_cursor.fetchone()
        while row:
            buildid, databaseid, ownerid, listid, listaction = (
                row[0],
                row[1],
                row[2],
                row[3],
            ...
            code_val = row[4].replace("-T", "").replace("-C", "")
            logging.info("buildid=%d, databaseid=%s" % (buildid, databaseid))

            insert_sql = f"""
                INSERT INTO ##Process_Apogee_Step02
                (BuildID, DatabaseID, OwnerID, ListID, TransListID, Code, ListAction, TransListAction) VALUES
                ({buildid}, {databaseid}, {ownerid}, {listid}, 0, '{code_val}', '{listaction}', '')
                """

            logging.info(insert_sql)
            result = sqlhook.get_first(
                f"select 1 from ##Process_Apogee_Step02 where code='{code_val}'"
            )
            if not result:
                sqlhook.run(insert_sql)
            else:
                sqlhook.run(
                    f"update ##Process_Apogee_Step02 SET TransListID={listid}, TransListAction='{listaction}' where Code ='{code_val}'"
                )

            row = sql_cursor.fetchone()

        sql_cursor.execute(
            "select * from ##Process_Apogee_Step02 where TransListID = 0 OR ListID = 0 OR ListAction<>TransListAction"
        )
        row = sql_cursor.fetchone()
        if row:
            logging.info(
                "Error Found. There are records with TransListID = 0 OR ListID = 0 OR Actions. Please check"
            )
            while row:
                logging.info(row)
                row = sql_cursor.fetchone()
            raise ValueError("Error found in input records")
        else:
            allsql = f"""drop table if exists apogee_input_count;
                    CREATE TABLE apogee_input_count (
                        OwnerID varchar (4) NOT NULL,
                        Type varchar (14) NULL,
                        tCount int NULL,
                        ListID bigint NOT NULL,
                        ListAction varchar (1) NULL); commit;
                        INSERT INTO apogee_input_count\n"""

            # CB: Changed DonTrans to TotalTrans
            sql_cursor.execute(
                "select buildid,DatabaseID,OwnerID,ListID,TransListID,Code, ListAction from ##Process_Apogee_Step02"
            )
            row = sql_cursor.fetchone()
            while row:
                databaseid, ownerid, listid, transactionid, listaction = (
                    row[1],
                    row[2],
                    row[3],
                    row[4],
                ...
                lcsql = f"""SELECT '{ownerid}' as ownerid, 'Donor' as type, COUNT(*)  as tCount, 
                            {listid} as ListId, '{listaction}' as ListAction 
                            from dw_final_{databaseid}_{buildid}_{listid} UNION
                            SELECT '{ownerid}' as ownerid, 'Distinct Donor' as type, COUNT(Distinct AccountNo) as tCount,
                            {listid} as ListId, '{listaction}' as ListAction 
                            from dw_final_{databaseid}_{buildid}_{listid} UNION
                            SELECT '{ownerid}' as ownerid, 'TotalTrans' as type, COUNT(*) as tCount, 
                            {transactionid} as ListId, '{listaction}' as ListAction  
                            from dw_final_{databaseid}_{buildid}_{transactionid} UNION\n"""

                allsql = allsql + lcsql + " "

                # CB: Changed NotNullComp to CompanyIDs
                lcSQL2 = f"""SELECT '{ownerid}' as ownerid, 'CompanyIDs'  as type, COUNT(*)  as tCount, 
                    {listid} as ListId, '{listaction}' as ListAction 
                    from dw_final_{databaseid}_{buildid}_{listid} where nvl(company, '')!='' UNION\n"""

                allsql = allsql + lcSQL2 + " "

                # CB: Changed xJoins to MatchingIDs
                lcSQL3 = f"""SELECT '{ownerid}' as ownerid, 'MatchingIDs'  as type, COUNT(*)  as tCount, 
                            {listid} as ListId, '{listaction}' as ListAction 
                            from dw_final_{databaseid}_{buildid}_{listid} a 
                            inner join dw_final_{databaseid}_{buildid}_{transactionid} b 
                            on a.AccountNo = b.AccountNo where nvl(a.company_id, '')!='' UNION\n"""

                allsql = allsql + lcSQL3
                row = sql_cursor.fetchone()
            ...
            #         row = redshift_cursor.fetchone()
            logging.info(
                "apogee preaudit input count table is created on redshift: apogee_input_count"
            )
...
def get_np_fec_list(buildid, databaseid, *args, **kwargs):
    ...
    redshift_hook.run(
        "DROP TABLE IF EXISTS apogee_list_values; CREATE TABLE apogee_list_values (LISTID INT,cValue  VARCHAR (200));"
    )
    ...
    with closing(redshift_conn.cursor()) as redshift_cursor:
        ...
        while row:
            logging.info(row)
            donor_listid, trans_listid = row[2], row[3]
            sql_insert_count = f"""
                insert into apogee_list_values (listid, cvalue)
                select {donor_listid}, count(*) from dw_final_{databaseid}_{buildid}_{donor_listid}
                where nvl(ListCategory04,'')!='';

                insert into apogee_list_values (listid, cvalue)
                select {trans_listid}, count(*) from dw_final_{databaseid}_{buildid}_{trans_listid}
                where nvl(ListCategory04,'')!='';
            """
            redshift_hook.run(sql_insert_count)
